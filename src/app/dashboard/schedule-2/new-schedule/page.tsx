...
const CompanyForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditMode) {
        await axios.put(`${process.env.NEXT_PUBLIC_APP_URL}/schedule-2/${id}`, payload);
      } else {
        await axios.post(`${process.env.NEXT_PUBLIC_APP_URL}/schedule-2`, payload);
      }

      router.push('/dashboard/schedule-2');
    } catch (err) {
  ...
  return (
    <Box sx={{ maxWidth: 1200, mx: 'auto', p: 3 }}>
      <Button
        startIcon={<ArrowBack />}
        onClick={() => router.push('/dashboard/schedule-2')}
        sx={{ mb: 2 }}
      >
        Back to Schedules
      </Button>
